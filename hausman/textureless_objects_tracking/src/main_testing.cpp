...
//  pcl::io::loadPCDFile("afterwards_on_top.pcd", *cloud_for_dense);
...
//  clustering.readFromFile("poses_final.txt",4);
//  clustering.buildLaplacian();
//  //i- number of the row
//  for(int i=1;i<clustering.features_number_;i++){
//
//	  for(int j=0;j<i;j++){
//
//		  if(clustering.laplacian_(i,j)==-1){
//			  for(int k=0;k<dense.cloud_operational_->size();k++){
//				  if(dense.cloud_operational_->points[k].f==i)
//					  dense.cloud_operational_->points[k].f=j;
//			  }
//		  }
////		  clustering.laplacian_[clustering.features_number_];
//
//	  }
//  }

//  	  pcl::io::savePCDFile("trajectory_clustered.pcd",*dense.cloud_operational_);
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr euclidian_cloud(new pcl::PointCloud<pcl::PointXYZRGB>);
...
//	  pcl::copyPointCloud(*dense.cloud_operational_,*euclidian_cloud);


  	  std::vector<int> feature_indices;
  	  for(int i=0; i< dense.cloud_operational_->size(); ++i)
  	    if(dense.cloud_operational_->points[i].f == 2)
  	      feature_indices.push_back(i);
	pcl::copyPointCloud(*dense.cloud_operational_,feature_indices,*euclidian_cloud)  ;
...
//	pcl::KdTreeFLANN<pcl::PointXYZRGB> kdtree;
	...
	std::cerr<<"Size of the cluster_indices: "<<cluster_indices.size()<<std::endl;

//  for (int repeat=1;repeat<2;repeat++){
	  for(int cluster_nu=0;cluster_nu<cluster_indices.size();cluster_nu++){
		  std::cerr<<"Size of one cluster from cluster indices: "<<cluster_indices[cluster_nu].indices.size()<<std::endl;
	  int fp_index=0;
//	  	  std::vector<int> feature_indices;
//	  	  for(int i=0; i< dense.cloud_operational_->size(); ++i)
//	  	    if(dense.cloud_operational_->points[i].f == repeat)
//	  	      feature_indices.push_back(i);
//	  	  float min_curv =std::numeric_limits<float>::max();

	  Eigen::Vector4f c;
//  	  pcl::compute3DCentroid<pcl::PointXYZLRegionF>(*dense.cloud_operational_,feature_indices, c);

	  pcl::PointIndices index;
	for (std::vector<int>::const_iterator pit = cluster_indices[cluster_nu].indices.begin (); pit != cluster_indices[cluster_nu].indices.end (); pit++)
	  index.indices.push_back(feature_indices[*pit]);

	pcl::compute3DCentroid<pcl::PointXYZLRegionF>(*dense.cloud_operational_,index, c);
  	  pcl::copyPointCloud(*dense.cloud_operational_,*segments);
	 ...
	 std::cerr<<"Number of indices with the feature number "<<1<<": "<<feature_indices.size()<<std::endl;

	 dense.normalsEstimation(dense.cloud_operational_,normals);

//	  for(int i=0; i<feature_indices.size(); ++i)
//	  {
//	    if(normals->points[feature_indices[i]].curvature < min_curv)
//	    {
//	      fp_index = feature_indices[i];
//	      min_curv = normals->points[feature_indices[i]].curvature;
//	      if(min_curv == 0) break;
//	    }
//	  }

  pcl::PointIndices indices_out;
  dense.activeSegmentation(*dense.cloud_operational_,0.01f,89,fp_index,indices_out);
  ...
  //save segment to pcd file if it contains more points the a certain threshold
    ...
    {
      cloud_segment->points.push_back (dense.cloud_operational_->points[*pit]);
      dense.cloud_operational_->points[*pit].reg = 1+1;
    }
...
//    writer.write<pcl::PointXYZLRegionF> ("result_cloud.pcd", *cloud_in, false);
  }

	for (std::vector<int>::const_iterator pit = cluster_indices[0].indices.begin (); pit != cluster_indices[0].indices.end (); pit++)
//	for (std::vector<int>::const_iterator pit = feature_indices.begin (); pit != feature_indices.end (); pit++)

		dense.cloud_operational_->points[feature_indices[*pit]].label=1;

    pcl::io::savePCDFile("RESULT.pcd",*dense.cloud_operational_);
...
//
//
//  for (int i=0;i<dense.clusters_vec_point_cloud.size();i++){
//
//	  pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_temp(new pcl::PointCloud<pcl::PointXYZRGBA>);
//
//	  pcl::copyPointCloud(*(dense.clusters_vec_point_cloud[i]),*cloud_temp);
//
//	  clusters_vec.push_back(cloud_temp);
//
//

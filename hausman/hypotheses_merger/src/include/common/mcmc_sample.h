...
namespace people {
...
#if 0
			std::cout << "number of targets "  << states_.size() << std::endl;
			std::cout << "number of interaction variables " << group_interaction_.size() << std::endl;
			std::cout << "request " << idx1 << " " << idx2 << std::endl;

			for(size_t i = 0; i < group_interaction_.size(); i++) {
				std::cout << i << " " << group_interaction_[i].size() << std::endl;
			}
#endif
			my_assert(idx1 < idx2);
	...
	public:
#ifdef VEL_STATE
		TargetDist():nframes_(1),sigma_x_(0.8),sigma_y_(0.8),sigma_z_(0.4),sigma_vx_(0.5),sigma_vy_(0.5),sigma_vz_(0.1),cov_(3,3,CV_32F){};
#else
		TargetDist():nframes_(1),sigma_x_(0.8),sigma_y_(0.8),sigma_z_(0.4),cov_(3,3,CV_32F){};
#endif
		virtual ~TargetDist(){};
		...
		inline cv::Mat getCov() {return cov_;};
		...
		virtual PeopleStatePtr drawSample(const double timestamp)
		{
			...
			double dt = timestamp - ret->timesec_; // uncertainty over time
			double vel_factor = get_vel_factor(nframes_);

			// not gonna allow x_
			ret->x_ += rng_.gaussian(sigma_x_ * dt);
			ret->y_ += rng_.gaussian(sigma_y_ * dt);
			ret->z_ += rng_.gaussian(sigma_z_ * dt);
			ret->vx_ += rng_.gaussian(sigma_vx_ * dt * vel_factor);
			ret->vy_ += rng_.gaussian(sigma_vy_ * dt * vel_factor);
			ret->vz_ += rng_.gaussian(sigma_vz_ * dt * vel_factor);
		...
		virtual double getSampleProbability(PeopleStatePtr state, double timestamp)
		{
			...
			assert(0);
#else
			double dt = timestamp - samples_[0]->timesec_; // uncertainty over time
			for(int i = 0; i < (int)samples_.size(); i++)
			{
				ret += (gaussian_prob(state->x_, samples_[i]->x_, sigma_x_ * dt)
							* gaussian_prob(state->y_, samples_[i]->y_, sigma_y_ * dt)
							* gaussian_prob(state->z_, samples_[i]->z_, sigma_z_ * dt)) / samples_.size();
			}
			ret += EPS;
#endif
			return ret;
	...
	protected:
		void updateStatistics(int maxSamples)
		{
			...
			if(N == 0)
			{
				mean_.x_ = mean_.y_ = mean_.z_ = 0.0;
#ifdef VEL_STATE
				mean_.vx_ = mean_.vy_ = mean_.vz_ = 0.0;
#endif
				mean_.confidence_ = 0.0;
				return;
			}

			mean_.confidence_ = (double)N / maxSamples;
			mean_.timesec_ = samples_[0]->timesec_;
			...
			for(int i = 0; i < N; i++)
			{
				mean_.x_ += samples_[i]->x_;
				mean_.y_ += samples_[i]->y_;
				mean_.z_ += samples_[i]->z_;
#ifdef VEL_STATE
				mean_.vx_ += samples_[i]->vx_;
				mean_.vy_ += samples_[i]->vy_;
				mean_.vz_ += samples_[i]->vz_;
#endif
			}
			mean_.x_ /= N;
			mean_.y_ /= N;
			...
			for(int i = 0; i < N; i++)
			{
				double dx = samples_[i]->x_ - mean_.x_;
				double dy = samples_[i]->y_ - mean_.y_;
				double dz = samples_[i]->z_ - mean_.z_;

				cov_.at<float>(0,0) += dx * dx / N;
				cov_.at<float>(0,1) += dx * dy / N;
				cov_.at<float>(0,2) += dx * dz / N;
				cov_.at<float>(1,0) += dy * dx / N;
				cov_.at<float>(1,1) += dy * dy / N;
				cov_.at<float>(1,2) += dy * dz / N;
				cov_.at<float>(2,0) += dz * dx / N;
				cov_.at<float>(2,1) += dz * dy / N;
				cov_.at<float>(2,2) += dz * dz / N;
			}
#endif
		};
	protected:
	...
	public:
		...
		void updateStatistics(int maxSamples)
		{
			...
			mean_.confidence_ = (double)N / maxSamples;
			mean_.timesec_ = feats_[0]->timesec_;

			mean_.x_ = mean_.y_ = mean_.z_ = 0.0;

			for(int i = 0; i < N; i++)
			{
				mean_.x_ += feats_[i]->x_;
				mean_.y_ += feats_[i]->y_;
				mean_.z_ += feats_[i]->z_;
			}
			mean_.x_ /= N;
			mean_.y_ /= N;
			mean_.z_ /= N;
	...
	public:
...
#if 1 // must be satisfied!!!!!!!!!
						if(ts > 0) {
							my_assert(ts == samples_[j]->getState(i)->timesec_);
						}
						else {
							ts = samples_[j]->getState(i)->timesec_;
						}
#endif
					}
				}
		...
		GFeatStatePtr drawFeatSample(int idx, double timestamp)
		{
			...
			GFeatStatePtr feat;
			int sidx = floor(rng_.uniform((double)0.0f, (double)samples_.size() - EPS));
			
			if(samples_[sidx]->getFeatValidity(idx)) 
				feat = samples_[sidx]->getFeatState(idx)->clone();

			feat->timesec_ = timestamp;
		...
		CamStatePtr getMeanCamera() 
		{
			...
			state->timesec_ = samples_[0]->getCamState()->timesec_;
			for(size_t i = 0; i < samples_.size(); i++) {
				CamStatePtr cam_sample = samples_[i]->getCamState();

				state->x_ += cam_sample->x_ / samples_.size();
				state->y_ += cam_sample->y_ / samples_.size();
				state->z_ += cam_sample->z_ / samples_.size();
				state->pitch_ += cam_sample->pitch_ / samples_.size();
				state->yaw_ += cam_sample->yaw_ / samples_.size();
				state->roll_ += cam_sample->roll_ / samples_.size();
				
				my_assert(state->timesec_ == cam_sample->timesec_);
			}
		...
		{
			return feat_dists_[idx]->getMean()->confidence_;
		...
		void removeTarget(int idx)
		{
			...
			for(size_t i = 0; i < samples_.size(); i++) {
#ifdef _DEBUG
				// std::cout << "handling sample" << i << std::endl;
				// fflush(stdout);
#endif

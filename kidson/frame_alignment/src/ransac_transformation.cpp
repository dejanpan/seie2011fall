...
void RansacTransformation::testfn()
{
	std::cout << "test\n";
}
//
...
//Eigen::Matrix4f RansacTransformation::getTransformFromMatches(const std::vector<Eigen::Vector4f> & source_feature_locations_3d,
...
//    transformation = getTransformFromMatches(source_feature_locations_3d, target_feature_locations_3d, sample_matches.begin(), sample_matches.end(),valid,max_dist_m);
...
//
//    transformation = getTransformFromMatches(source_feature_locations_3d, target_feature_locations_3d, matches.begin(), matches.end(), valid); // compute new trafo from all inliers:
//    if(transformation!=transformation) continue; //Contains NaN

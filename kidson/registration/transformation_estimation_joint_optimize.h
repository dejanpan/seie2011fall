/*
 * TransformationEstimationJointOptimize.h
 *
 *  Created on: May 8, 2012
 *      Author: rossk
 */

#ifndef TRANSFORMATION_ESTIMATION_JOINT_OPTIMIZE_H
#define TRANSFORMATION_ESTIMATION_JOINT_OPTIMIZE_H
...
class TransformationEstimationJointOptimize: public pcl::registration::TransformationEstimation<PointSource, PointTarget> {
	...
	float denseCloudWeight; //<! weight used in distance weighting of the points
	float visualFeatureWeight; //<! weight given to term in optimization equation
...
public:
	...
	TransformationEstimationJointOptimize() {
		...
		denseCloudWeight = 0.4;
		visualFeatureWeight = 0.3;
		handleFeatureWeight = 0.3;
		indices_src_dfp_set_ = false;
		indices_tgt_dfp_set_ = false;
		weights_dfp_set_ = false;
	};

	virtual ~TransformationEstimationJointOptimize() {};

	inline void
	setWeights(float denseWeight, float visualWeight, float handleWeight);
...
//		return (0.8*0.8)/(depth*depth);
	...
	/** \brief Temporary pointer to the source dataset distinctive feature point indices. */
	const std::vector<int> *tmp_idx_src_handles_;

	/** \brief Temporary pointer to the target dataset distinctive feature point indices. */
	const std::vector<int> *tmp_idx_tgt_handles_;
	...
	struct OptimizationFunctor : Functor<double>
	{
		...
		TransformationEstimationJointOptimize<PointSource, PointTarget> *estimator_;
	};

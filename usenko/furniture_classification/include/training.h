...
bool operator <(const Histogram<N> & f, const Histogram<N> & s) {
	std::vector<float> v_f(f.histogram, f.histogram + N), v_s(s.histogram,
			s.histogram + N);
	return v_f < v_s;
}
}
...
typedef std::map<featureType, std::map<std::string, pcl::PointCloud<
		pcl::PointXYZ> > > databaseType;
...
YAML::Emitter& operator <<(YAML::Emitter& out,
	...
	for (size_t i = 0; i < cloud.points.size(); i++) {
		out << YAML::Flow << YAML::BeginSeq << cloud.points[i].x
				<< cloud.points[i].y << cloud.points[i].z << YAML::EndSeq
				<< YAML::Block;
	}
	out << YAML::EndSeq;
...
void normalizeFeatures(std::vector<pcl::Histogram<N> > & features,
		pcl::Histogram<N> & min, pcl::Histogram<N> & max) {
	...
	for (size_t i = 0; i < features.size(); i++) {
		for (int j = 0; j < N; j++) {
			features[i].histogram[j] = (features[i].histogram[j]
					- min.histogram[j]) / (max.histogram[j] - min.histogram[j]);
		}
	}
...
template<int N>
void normalizeFeatures(std::vector<pcl::Histogram<N> > & features,
		pcl::Histogram<N> & min, pcl::Histogram<N> & max, const pcl::Histogram<
				N> & min_train, const pcl::Histogram<N> & max_train) {

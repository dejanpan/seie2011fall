...
void addNoise(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double noise_std) {
	...
	boost::normal_distribution<float> normal_distrib(0.0f, noise_std
			* noise_std);
...
void moveToNewCenterAndAlign(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
		...
		double new_center[3], double tilt_angle) {

	Eigen::Affine3f view_transform;
	view_transform.setIdentity();
	Eigen::Translation<float, 3> translation(new_center[1], new_center[2],
			-new_center[0]);
	Eigen::AngleAxis<float> tilt_rotation(tilt_angle * M_PI / 180 + M_PI,
...
inline void randomPointTriangle(float a1, float a2, float a3, float b1,
		float b2, float b3, float c1, float c2, float c3, Eigen::Vector4f& p) {
...
inline void randPSurface(vtkPolyData * polydata,
	...
	std::vector<double>::iterator low = std::lower_bound(
			cumulativeAreas->begin(), cumulativeAreas->end(), r);
	...
	randomPointTriangle(float(A[0]), float(A[1]), float(A[2]), float(B[0]),
			float(B[1]), float(B[2]), float(C[0]), float(C[1]), float(C[2]), p);
}
...
void createFullModelPointcloud(vtkSmartPointer<vtkPolyData> polydata,
		size_t n_samples, pcl::PointCloud<pcl::PointXYZ> & cloud_out) {

	vtkSmartPointer<vtkTriangleFilter> triangleFilter = vtkSmartPointer<
			vtkTriangleFilter>::New();
	triangleFilter->SetInput(polydata);
	triangleFilter->Update();

	vtkSmartPointer<vtkPolyDataMapper> triangleMapper = vtkSmartPointer<
...
int main(int argc, char** argv) {
	...
	for (size_t i = 0; i < files_to_process.size(); i++) {
		...
		boost::split(st, files_to_process.at(i), boost::is_any_of("/"),
		...
		for (size_t shift_index = 0; shift_index < shift.size(); shift_index++) {
			...
			for (size_t tilt_index = 0; tilt_index < tilt.size(); tilt_index++) {
				...
				for (size_t distance_index = 0; distance_index
						< distances.size(); distance_index++) {
					...
					for (int angle_index = 0; angle_index < num_views; angle_index++) {
						...
						transform->Translate(distances[distance_index],
								shift[shift_index], -(height + min_z_value));
						...
						ss << dirname << "/rotation" << angle << "_distance"
								<< distances[distance_index] << "_tilt"
